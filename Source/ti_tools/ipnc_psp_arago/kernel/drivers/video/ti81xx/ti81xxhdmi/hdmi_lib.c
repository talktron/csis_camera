...
#include <linux/ti81xx/video/ti81xxhdmi.h>
#include <plat/cpu.h>
...
static inline void hdmi_write_reg(void __iomem *base, u16 idx, u32 val)
{
	void __iomem *b = base;
	__raw_writel(val, b + idx);
}

static inline u32 hdmi_read_reg(void __iomem *base, u16 idx)
{
	void __iomem *b = base;
	u32 l;
	l = __raw_readl(b + idx);
...
int hdmi_core_ddc_edid(u8 *pEDID, int ext)
{
	...
	void __iomem *ins = hdmi.base_core;
	u32 offset = 0;
	void __iomem *base;

	base = hdmi.base_core_av;
	/* Turn on CLK for DDC */
...
static int hdmi_core_video_config(struct hdmi_core_video_config_t *cfg)
{
	...
	/* For TI814x, set the SYNC based on embedded or discrete
	 */
	if (soc_is_ti814x()) {
				...
				struct hdmi_core_infoframe_avi info_avi)
{
	...
	hdmi_write_reg(hdmi.base_core_av, HDMI_CORE_AV_AVI_TYPE, 0x082);
	hdmi_write_reg(hdmi.base_core_av, HDMI_CORE_AV_AVI_VERS, 0x002);
	hdmi_write_reg(hdmi.base_core_av, HDMI_CORE_AV_AVI_LEN, 0x00D);
	...
	val = (info_avi.db1y_rgb_yuv422_yuv444 << 5) |
		...
		(info_avi.db1s_0_1_2);
	hdmi_write_reg(hdmi.base_core_av, offset, val);
	sum += val;
	...
	hdmi_write_reg(hdmi.base_core_av, offset, info_avi.db4vic_videocode);
	...
	checksum = 0x100 - sum;
	hdmi_write_reg(hdmi.base_core_av, HDMI_CORE_AV_AVI_CHSUM, checksum);
...
int hdmi_configure_csc(enum hdmi_core_av_csc csc)
{
	...
	case RGB:
		...
		var = hdmi_read_reg(hdmi.base_core, HDMI_CORE_SYS__VID_ACEN);
		var = FLD_MOD(var, 0, 2, 2);
		hdmi_write_reg(hdmi.base_core, HDMI_CORE_SYS__VID_ACEN, var);
		break;
	case RGB_TO_YUV:
		hdmi.avi_param.db1y_rgb_yuv422_yuv444 =
	...
	case YUV_TO_RGB:
		hdmi.avi_param.db1y_rgb_yuv422_yuv444 = INFOFRAME_AVI_DB1Y_RGB;
		hdmi_core_audio_infoframe_avi(hdmi.avi_param);
		var = hdmi_read_reg(hdmi.base_core, HDMI_CORE_SYS__VID_MODE);
		var = FLD_MOD(var, 1, 3, 3);
		hdmi_write_reg(hdmi.base_core, HDMI_CORE_SYS__VID_MODE, var);
		break;
	default:
		break;
	}
...
int hdmi_configure_lrfr(enum hdmi_range lr_fr, int force_set)
{
	...
	case HDMI_LIMITED_RANGE:
		...
		hdmi_core_audio_infoframe_avi(hdmi.avi_param);
		if (force_set) {
			var = hdmi_read_reg(hdmi.base_core,
						HDMI_CORE_SYS__VID_ACEN);
			var = FLD_MOD(var, 1, 1, 1);
			hdmi_write_reg(hdmi.base_core,
...
static int hdmi_core_av_packet_config(void __iomem *name,
	struct hdmi_core_packet_enable_repeat r_p)
{
...
static void hdmi_w1_irq_enable(struct hdmi_irq_vector *pIrqVectorEnable)
{
	...
	hdmi_write_reg(hdmi.base_wp, HDMI_WP_IRQENABLE_SET, r);
}
...
static void hdmi_w1_irq_wakeup_enable(struct hdmi_irq_vector *pIrqVectorEnable)
{
	...
	r = ((pIrqVectorEnable->phyDisconnect << 26) |
			(pIrqVectorEnable->phyConnect << 25) |
			(pIrqVectorEnable->core << 0));
	hdmi_write_reg(hdmi.base_wp, HDMI_WP_IRQWAKEEN, r);
}

static inline int hdmi_w1_wait_for_bit_change(void __iomem *ins,
...
{
	/* reset W1 */
	REG_FLD_MOD(hdmi.base_wp, HDMI_WP_SYSCONFIG, 0x1, 0, 0);

	/* wait till SOFTRESET == 0 */
	while (FLD_GET(hdmi_read_reg(hdmi.base_wp, HDMI_WP_SYSCONFIG), 0, 0))
		;
	return 0;
}
...
int hdmi_w1_set_wait_phy_pwr(HDMI_PhyPwr_t val)
{
	...
	REG_FLD_MOD(hdmi.base_wp, HDMI_WP_PWR_CTRL, val, 7, 6);

	if (hdmi_w1_wait_for_bit_change(hdmi.base_wp, HDMI_WP_PWR_CTRL, 5, 4, val)
	    != val) {
...
{
	REG_FLD_MOD(hdmi.base_wp, HDMI_WP_PWR_CTRL, val, 3, 2);

	/* wait till PHY_PWR_STATUS=ON */
	if (hdmi_w1_wait_for_bit_change(hdmi.base_wp, HDMI_WP_PWR_CTRL, 1, 0, val)
	    != val) {
...
{
	REG_FLD_MOD(hdmi.base_wp, HDMI_WP_VIDEO_CFG, 0, 31, 31);
}
void hdmi_w1_video_start(void)
{
	/* For TI814x some secret bit needs to be set */
	if (soc_is_ti814x())
		REG_FLD_MOD(hdmi.base_wp, HDMI_WP_VIDEO_CFG,
					(u32)0x1, 2, 2);
	REG_FLD_MOD(hdmi.base_wp, HDMI_WP_VIDEO_CFG, (u32)0x1, 31, 31);
}

unsigned int hdmi_w1_video_status(void)
{
	return 	(hdmi_read_reg(hdmi.base_wp, HDMI_WP_VIDEO_CFG) & 0x80000000);
}
	...
	struct hdmi_video_format *f_p)
{
	...
	REG_FLD_MOD(hdmi.base_wp, HDMI_WP_VIDEO_CFG, f_p->packingMode, 10, 8);

	l |= FLD_VAL(f_p->linePerPanel, 31, 16);
	l |= FLD_VAL(f_p->pixelPerLine, 15, 0);
	hdmi_write_reg(hdmi.base_wp, HDMI_WP_VIDEO_SIZE, l);
}
	...
	struct hdmi_video_interface *i_p)
{
	...
	r = hdmi_read_reg(hdmi.base_wp, HDMI_WP_VIDEO_CFG);
	...
	hdmi_write_reg(hdmi.base_wp, HDMI_WP_VIDEO_CFG, r);
}
		...
		struct hdmi_video_timing *t_p)
{
	...
	hdmi_write_reg(hdmi.base_core, HDMI_CORE_SYS__HBIT_2HSYNC1, hbit2_hsync1);
	hdmi_write_reg(hdmi.base_core, HDMI_CORE_SYS__HBIT_2HSYNC2, hbit2_hsync2);
	hdmi_write_reg(hdmi.base_core, HDMI_CORE_SYS__FLD2_HS_OFSTL, fld2_hs_ofstl);
	hdmi_write_reg(hdmi.base_core, HDMI_CORE_SYS__FLD2_HS_OFSTH, fld2_hs_ofsth);
	hdmi_write_reg(hdmi.base_core, HDMI_CORE_SYS__HWIDTH1, hwidth1);
	hdmi_write_reg(hdmi.base_core, HDMI_CORE_SYS__HWIDTH2, hwidth2);
	hdmi_write_reg(hdmi.base_core, HDMI_CORE_SYS__VBIT_TO_VSYNC, vbit2_vsync);
	hdmi_write_reg(hdmi.base_core, HDMI_CORE_SYS__VWIDTH, vwidth);
}
...
int hdmi_lib_enable(struct hdmi_config *cfg)
{
	...
	/*
	   ...
	   is met.
	 */
	hdmi_write_reg(hdmi.base_wp, HDMI_WP_WP_CLK, 0x100);
	...
	/* TI814x works in both discrete and embedded sync mode.
...
int hdmi_lib_init(unsigned long ti81xx_hdmi_wp, unsigned int ti81xx_hdmi_wp_size) {
	u32 rev;
	if(soc_is_ti814x()){
		hdmi.base_wp = ioremap(ti81xx_hdmi_wp, ti81xx_hdmi_wp_size);
	} else {
		/* Can't MAP memory region on this CPU */
		return -ENOMEM;
	}
	if (!hdmi.base_wp) {
	...
	rev = hdmi_read_reg(hdmi.base_wp, HDMI_WP_REVISION);
	printk(KERN_INFO "HDMI W1 rev %d.%d\n",
		FLD_GET(rev, 10, 8), FLD_GET(rev, 5, 0));
	rev = hdmi_read_reg(hdmi.base_core_cec, HDMI_CORE_CEC_SPEC);
...
int hdmi_set_irqs(int i)
{
	...
	if (!i) {
		...
		r = hdmi_read_reg(hdmi.base_wp, HDMI_WP_IRQENABLE_SET);
		DBG("Irqenable %x\n", r);
	}
	r = hdmi_read_reg(hdmi.base_core, HDMI_CORE_CTRL1);
	/* PD bit has to be written to recieve the interrupts */
	r = FLD_MOD(r, !i, 0, 0);
	hdmi_write_reg(hdmi.base_core, HDMI_CORE_CTRL1, r);

	hdmi_write_reg(hdmi.base_core, HDMI_CORE_SYS__UMASK1, i ? 0x00 : 0x40);
	hpd = hdmi_read_reg(hdmi.base_core, HDMI_CORE_SYS__UMASK1);
	DBG("%x hpd\n", hpd);
...
void HDMI_W1_HPD_handler(int *r)
{
	...
	DBG("%x hdmi_wp_irqstatus\n", \
		hdmi_read_reg(hdmi.base_wp, HDMI_WP_IRQSTATUS));
	DBG("%x hdmi_core_intr_state\n", \
		hdmi_read_reg(hdmi.base_core, HDMI_CORE_SYS__INTR_STATE));
	DBG("%x hdmi_core_intr1\n", \
		hdmi_read_reg(hdmi.base_core, HDMI_CORE_SYS__INTR1));
	DBG("%x hdmi_core_intr2\n", \
		hdmi_read_reg(hdmi.base_core, HDMI_CORE_SYS__INTR2));
	DBG("%x hdmi_core_intr3\n", \
		hdmi_read_reg(hdmi.base_core, HDMI_CORE_SYS__INTR3));
	DBG("%x hdmi_core_sys_sys_stat\n", \
		hdmi_read_reg(hdmi.base_core, HDMI_CORE_SYS__SYS_STAT));
	DBG("-------------DEBUG-------------------\n");

	val = hdmi_read_reg(hdmi.base_wp, HDMI_WP_IRQSTATUS);
	if (val & HDMI_WP_IRQSTATUS_CORE) {
		...
		if (core_state & 0x1) {
			set =
			hdmi_read_reg(hdmi.base_core, HDMI_CORE_SYS__SYS_STAT);
			hpd_intr = hdmi_read_reg(hdmi.base_core,
					HDMI_CORE_SYS__INTR1);
			intr2 = hdmi_read_reg(hdmi.base_core,
					HDMI_CORE_SYS__INTR2);
			intr3 = hdmi_read_reg(hdmi.base_core,
					HDMI_CORE_SYS__INTR3);

			hdmi_write_reg(hdmi.base_core, HDMI_CORE_SYS__INTR1,
					hpd_intr);
			hdmi_write_reg(hdmi.base_core, HDMI_CORE_SYS__INTR2,
					intr2);
			hdmi_write_reg(hdmi.base_core, HDMI_CORE_SYS__INTR3,
					intr3);
	...
	hdmi_write_reg(hdmi.base_wp, HDMI_WP_IRQSTATUS, val);
	/* flush posted write */
	hdmi_read_reg(hdmi.base_wp, HDMI_WP_IRQSTATUS);
}
...
int HDMI_CORE_DDC_READEDID(void __iomem * name, u8 *p, u16 max_length)
{
	int r = read_edid(p, max_length);
	return r;
}

int HDMI_W1_StopVideoFrame(void __iomem * name)
{
...
int HDMI_W1_StartVideoFrame(void __iomem *name)
{
...
/* PHY_PWR_CMD */
int HDMI_W1_SetWaitPhyPwrState(HDMI_PhyPwr_t param)
{
...
/* PLL_PWR_CMD */
int HDMI_W1_SetWaitPllPwrState(void __iomem *name,
		HDMI_PllPwr_t param)
{
...
{
	int state = hdmi_read_reg(hdmi.base_core, HDMI_CORE_SYS__SYS_STAT);
...
int hdmi_lib_cec_activate(void)
{
	...
	temp = hdmi_read_reg(hdmi.base_core_cec, HDMI_CORE_CEC_SET_UP);
	if (FLD_GET(temp, 4, 4) != 0) {
		temp = FLD_MOD(temp, 0, 4, 4);
		hdmi_write_reg(hdmi.base_core_cec, HDMI_CORE_CEC_SET_UP, temp);
		...
		temp = hdmi_read_reg(hdmi.base_core_cec, HDMI_CORE_CEC_INT_STATUS_1);
		temp = FLD_MOD(0x0, 0x5, 2, 0);
		hdmi_write_reg(hdmi.base_core_cec, HDMI_CORE_CEC_INT_STATUS_1, temp);
	}
	return 0x0;
}
...
int hdmi_lib_cec_reg_dev(struct ti81xxhdmi_cec_register_dev *dev, __u8 *pa)
{
	...
	*/
	...
	old_ca_15_8 = hdmi_read_reg(hdmi.base_core_cec, HDMI_CORE_CEC_CA_15_8);
	old_ca_7_0 = hdmi_read_reg(hdmi.base_core_cec, HDMI_CORE_CEC_CA_7_0);
	...
	if (wr_msg.acked_nacked != 1) {
		...
		if (dev->clear_registered_dev == 0x1) {
			hdmi_write_reg(hdmi.base_core_cec, HDMI_CORE_CEC_CA_7_0, 0);
			hdmi_write_reg(hdmi.base_core_cec, HDMI_CORE_CEC_CA_15_8, 0);
		} else {
			temp = hdmi_read_reg(hdmi.base_core_cec, regis_reg);
		}
		temp |= 0x1 << shift_cnt;
		hdmi_write_reg(hdmi.base_core_cec, regis_reg, temp);
		...
		if ((wr_msg.acked_nacked != 1) || (r != 0x0)) {
			...
			hdmi_write_reg(hdmi.base_core_cec,
...
int hdmi_lib_cec_write_msg(struct ti81xxhdmi_cec_transmit_msg *msg)
{
	...
	/* Clear TX FIFO */
	temp = hdmi_read_reg(hdmi.base_core_cec, HDMI_CORE_CEC_DBG_3);
	temp = FLD_MOD(temp, 1, 7, 7);
	hdmi_write_reg(hdmi.base_core_cec, HDMI_CORE_CEC_DBG_3, temp);
	...
	temp = hdmi_read_reg(hdmi.base_core_cec, HDMI_CORE_CEC_INT_STATUS_0);
	if (FLD_GET(temp, 5, 5) == 1) {
		temp = FLD_MOD(0x0, 1, 5, 5);
		printk(KERN_INFO "HDMI CECDBG TX INT STAT0 value - %x\n", temp);
		hdmi_write_reg(hdmi.base_core_cec, HDMI_CORE_CEC_INT_STATUS_0, temp);
	}

	/* Set the initiator detination addresses */
	temp = hdmi_read_reg(hdmi.base_core_cec, HDMI_CORE_CEC_TX_INIT);
	temp = FLD_MOD(temp, msg->initiator, 3, 0);
	hdmi_write_reg(hdmi.base_core_cec, HDMI_CORE_CEC_TX_INIT, temp);

	temp = hdmi_read_reg(hdmi.base_core_cec, HDMI_CORE_CEC_TX_DEST);
	temp = FLD_MOD(temp, msg->destination, 3, 0);
	hdmi_write_reg(hdmi.base_core_cec, HDMI_CORE_CEC_TX_DEST, temp);
	...
	if (msg->send_ping)
	{
		...
		temp = hdmi_read_reg(hdmi.base_core_cec, HDMI_CORE_CEC_TX_COMMAND);
		temp = FLD_MOD(temp, msg->cmd, 7, 0);
		hdmi_write_reg(hdmi.base_core_cec, HDMI_CORE_CEC_TX_COMMAND, temp);


		for (i = 0; i < msg->no_args; i++) {
			temp = hdmi_read_reg(hdmi.base_core_cec,
		...
		/* Operand count */
		temp = hdmi_read_reg(hdmi.base_core_cec, HDMI_CORE_CEC_TRANSMIT_DATA);
		temp = FLD_MOD(temp, msg->no_args, 3, 0);
		/* Transmit */
		temp = FLD_MOD(temp, 0x1, 4, 4);
		hdmi_write_reg(hdmi.base_core_cec, HDMI_CORE_CEC_TRANSMIT_DATA, temp);
	}
...
int hdmi_lib_cec_read_msg(struct ti81xxhdmi_cec_received_msg *msg)
{
	...
	*/
	...
	temp = hdmi_read_reg(hdmi.base_core_cec, HDMI_CORE_CEC_INT_STATUS_0);
	if (FLD_GET(temp, 1, 1)) {
		cmd_cnt = hdmi_read_reg(hdmi.base_core_cec, HDMI_CORE_CEC_RX_COUNT);
		...
		temp = hdmi_read_reg(hdmi.base_core_cec, HDMI_CORE_CEC_RX_CMD_HEAD);
		msg->initiator = FLD_GET(temp, 7, 4);
		msg->destination = FLD_GET(temp, 3, 0);

		/* Get the command and operands */
		temp = hdmi_read_reg(hdmi.base_core_cec, HDMI_CORE_CEC_RX_COMMAND);
		...
		temp = hdmi_read_reg(hdmi.base_core_cec, HDMI_CORE_CEC_RX_CONTROL);
